use crate::{
    ...
    jit_compiler::{
        compile_inline_call_without_tick,
        module::{
...
impl<'context> InlineExpression<'context> for CompilableStructApplication<'_> {
    fn compile(
        ...
        Ok(
            // If the struct application was generated by the analyzer phase for a closure, return a
            // function instead of a struct value
            if let AnalyzedType::Function(function_type) = &self.0.type_ {
                UlarValue::Function(UlarFunction::Closure {
                    context_pointer: struct_pointer,
                    type_: function_type
                        .closure_type(compiler.context)
                        // If the function were unconvertible to an Inkwell type, we would've already
                        // returned an error
                        .unwrap(),
                })
            } else {
                UlarValue::Struct(struct_pointer)
            },
        )
    }
}
...
impl<'context> JitFunctionCompiler<'_, 'context> {
    ...
    ) -> Result<Box<dyn CompilableExpression<'b, 'context> + 'b>, CompilationError>
    where
        'context: 'b,
    {
        match expression {
            AnalyzedExpression::If(if_expression) => Ok(Box::new(CompilableIf(if_expression))),
            AnalyzedExpression::InfixOperation(infix_operation) => {
                Ok(Box::new(CompilableInfixOperation(infix_operation)))
            }

            AnalyzedExpression::Select(select) => Ok(Box::new(CompilableSelect(select))),
            AnalyzedExpression::Call(call) => Ok(Box::new(CompilableCall(call))),
            AnalyzedExpression::Closure(_) => Err(CompilationError {
                message: CompilationErrorMessage::InternalError(
                    InternalError::JitCompilerUnexpectedClosure,
                ),
            ...
            AnalyzedExpression::StructApplication(struct_application) => {
                Ok(Box::new(CompilableStructApplication(struct_application)))
            }

            AnalyzedExpression::PrefixOperation(prefix_operation) => {
                Ok(Box::new(CompilablePrefixOperation(prefix_operation)))
            }

            AnalyzedExpression::String(string_literal) => {
                Ok(Box::new(CompilableStringLiteral(string_literal)))
            }
        }
    }
    ...
    pub(super) fn compile_expression_graph(
        ...
        for (i, node) in expression_graph.node_iter() {
            compilables.insert(i, Self::compile_expression(node)?);
        }
    ...
    fn compile_inline_call(
        ...
        let function_pointer_pointer_name = scope.get_local_name();
        let function_pointer_name = scope.get_local_name();
        let result_name = scope.get_local_name();
        let result = compile_inline_call_without_tick(
            self.context,
            builder,
            function,
            scope.worker,
            arguments
                .iter()
                .map(|&argument| argument.try_into())
                .collect::<Result<Vec<_>, _>>()?,
            &function_pointer_pointer_name.to_string(),
            &function_pointer_name.to_string(),
            &result_name.to_string(),
        );

        UlarValue::from_call_site_value(self.context, type_, result, position)
    }
}
